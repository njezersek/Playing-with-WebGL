...
export default class Terrain{
	...
	height(x: number, y: number){
		...
		v += this.noise.noise2D(x,y)**2 * 0.9 * mask;
		v += this.noise.noise2D(x*2,y*2)**2 * 1.5 * mask;
		v += this.noise.noise2D(x*10,y*10) * 0.01;
		return v;
	}
	...
	render(dt: number, t: number){
		...
		mat4.translate(this.modelMatrix, this.modelMatrix, [-2, -1, -10]);
